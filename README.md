...
trường hợp của bài toán. Khí đó, ta chắn chắn tìm được đáp án chính xác.</p>
...
sử không bị vô hiệu hóa nếu nhập sai).</p>
...
- Solution:
  ...
  mỗi lần lặp, phải so sánh tổng vừa tìm được với tổng đang lưu (ketqua).</p>
  ...
  hiểu là vị trí phần tử đầu tiên trong dãy).</p>
  + Cuối cùng, ta xuất ra M phần tử từ vị trí đã lưu. Đây là đáp án cần tìm.
  ⟶ Với 2 vòng lặp lồng nhau thì độ phức tạp của bài toán là O(N*M). </p>
- Code mẫu C++:
  ```c++
     #include <bits/stdc++.h>
    using namespace std;
